    ...
    return (
        <div className='formContainer' onKeyPress={onKeyPress}>
            ...
            <div className='inputWrapper'>
                <input type='text' name='username' className='fieldInput' value={form.username} placeholder={'Nome de usuário'}
                       onChange={handleFormChange}/>
                {usernameError ? <span className='errorSpan'>Nome de usuário inválido</span> : null}
            </div>
            <div className='inputWrapper'>
                <input type='email' name='email' className='fieldInput' value={form.email} placeholder={'E-mail'}
                       onChange={handleFormChange}/>
            ...
            <div className='inputWrapper'>
                <div>
                    <input type={passwordShown ? 'text' : 'password'} name='password' className='fieldInput' value={form.password}
                           placeholder={'Senha'} onChange={handleFormChange} maxLength={6}/>
                    {passwordShown ?
                ...
                {passwordError ? <span className='errorSpan'>Senha inválida</span> : null}
            </div>
            <div className='inputWrapper'>
                <input type='text' className='fieldInput' name='schoolName' value={form.schoolName}
                       placeholder={'Nome da sua escola quando criança'} maxLength={20}
                       onChange={handleFormChange}/>
                {schoolNameError ? <span className='errorSpan'>Nome de escola inválido</span> : null}
            </div>

            <button disabled={submit} className='styledButton' onClick={Registration}>Cadastrar</button>
            <Link to='/login' className='haveAccountP'>Você não tem uma conta? Cadastrar</Link>
